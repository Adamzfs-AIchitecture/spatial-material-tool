...
function RoomModel({
  ...
  // Split Inspect mode mounts the same GLB twice.
  // Each Canvas needs its own cloned scene.
...
function InspectViewLine({ inspectPreset, inspectView }) {
  ...
  const isSideView = inspectView === "left" || inspectView === "right"

  // Important:
  // Front / Rear views look along Z, so their guide line runs across X at fixed Z.
  // Left / Right views look along X, so their guide line runs across Z at fixed X.
  const markerA = useMemo(() => {
    return isSideView
      ? new THREE.Vector3(px, y, -11.5)
      : new THREE.Vector3(-8.2, y, pz)
  }, [isSideView, px, pz])
  ...
  const lineGeometry = useMemo(() => {
    ...
    g.setFromPoints([markerA, markerB])
    return g
  }, [markerA, markerB])

  const mid = useMemo(() => {
    return new THREE.Vector3().addVectors(markerA, markerB).multiplyScalar(0.5)
  }, [markerA, markerB])

  const arrow = useMemo(() => {
    const origin = mid.clone()
    origin.y += 0.08
  ...
  return (
    <group>
      ...
      <mesh position={[markerA.x, y, markerA.z]} renderOrder={998}>
        <boxGeometry
          args={isSideView ? [1.0, 0.08, 0.08] : [0.08, 0.08, 1.0]}
        />
        <meshBasicMaterial color="#ff2d2d" depthTest={false} />
      </mesh>

      <mesh position={[markerB.x, y, markerB.z]} renderOrder={998}>
        <boxGeometry
          args={isSideView ? [1.0, 0.08, 0.08] : [0.08, 0.08, 1.0]}
        />
        <meshBasicMaterial color="#ff2d2d" depthTest={false} />
...
function SceneCanvas({
  ...
  return (
    <Canvas onPointerMissed={onPointerMissed}>
      ...
      <OrthographicCamera
        makeDefault={makeInspectDefault}
        near={0.1}
        far={2000}
      />
    ...
    >
      ...
      {mode === "inspect" && (
        <button
          ...
          title={
            showInspectContext
              ? "Hide spatial view guide"
              : "Show spatial view guide"
          }
        >
